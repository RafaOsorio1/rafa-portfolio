  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-40" style={{
      ...
      <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        <button onClick={handleLogoClick} className="font-mono text-sm font-bold tracking-widest hover:opacity-80 transition-opacity select-none" style={{ color: "#00E5FF" }}>
          &lt;DEV /&gt;
        </button>
        ...
        {konamiProgress > 0 && (
          <div className="hidden md:flex gap-1 absolute left-1/2 -translate-x-1/2">
            {KONAMI.map((_, i) => (
              <div key={i} className="w-1.5 h-1.5 rounded-full transition-all" style={{ background: i < konamiProgress ? "#00E5FF" : "rgba(0,229,255,0.15)" }} />
            ))}
          </div>
        )}
