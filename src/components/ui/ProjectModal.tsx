    ...
    >
      <div
        className={`relative w-full transition-all duration-300 ${
          isMaximized && !isMinimized ? 'max-w-none h-screen rounded-none' : 'max-w-2xl rounded-xl'
        } ${isMinimized ? 'overflow-hidden' : 'max-h-[90vh] overflow-y-auto'}`}
      ...
      >
        ...
        <div
          ...
          style={{ background: '#0C0C1A', borderBottom: '1px solid rgba(255,255,255,0.06)' }}
        >
          <button
            onClick={onClose}
          ...
          />
          <div
            className="flex-1 mx-3 px-3 py-1 rounded text-xs font-mono text-center"
            style={{
          ...
          <div className="animate-fade-up" style={{ animationDuration: '0.2s' }}>
            ...
            <div className="relative h-52 overflow-hidden">
              ...
              <div
                ...
                style={{ background: 'linear-gradient(to top, #0C0C1A 5%, transparent 55%)' }}
              />
              <div className="absolute bottom-4 left-6">
                <h2
                  className="text-3xl font-black"
                  style={{ fontFamily: "'Exo 2', sans-serif", color: project.accent }}
                >
                  {project.title}
                </h2>
            ...
            <div className="p-6 space-y-5">
              {'collaboration' in project && project.collaboration && (
                <div
                  className="inline-block px-3 py-1.5 rounded font-mono text-xs font-bold"
              ...
              <div className="flex flex-wrap gap-3 pt-1">
                ...
                {project.repo && (
                  ...
                  >
                    <Github size={13} />{' '}
                    {!('repoBack' in project) || !project.repoBack ? 'Código' : 'Frontend'}
                  </a>
                )}

                {'repoBack' in project && project.repoBack && (
                  <a
                    href={project.repoBack as string}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 px-5 py-2.5 rounded text-sm font-mono transition-opacity hover:opacity-70"
                    style={{ border: `1px solid ${project.accent}40`, color: project.accent }}
