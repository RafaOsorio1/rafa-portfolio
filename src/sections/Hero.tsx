...
export function Hero() {
  ...
  useEffect(() => {
    ...
    if (!isDeleting && typedText === text) {
      ...
      timeout = setTimeout(
        () => {
          setTypedText(
            isDeleting ? text.slice(0, typedText.length - 1) : text.slice(0, typedText.length + 1)
          );
        },
        isDeleting ? 45 : 75
      );
  ...
  return (
    <section
      id="hero"
      className="min-h-screen flex flex-col items-center justify-center relative pt-14 px-6"
    >
      <div className="max-w-4xl mx-auto text-center space-y-8 animate-fade-up">
        <div
          className="inline-flex items-center gap-2.5 font-mono text-xs px-4 py-1.5 rounded-full"
          style={{
        ...
        <div className="flex items-center justify-center gap-3 flex-wrap">
          <a
            href="#projects"
            className="px-6 py-2.5 rounded font-mono text-sm font-bold transition-all hover:opacity-90 hover:scale-[1.03]"
            style={{ background: '#00E5FF', color: '#06060E' }}
          >
            {t('hero.btn_projects') || 'ver_proyectos()'}
          </a>
          <a
            href="#contact"
            className="px-6 py-2.5 rounded font-mono text-sm font-bold transition-all hover:opacity-70"
