    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-3">
        <span className="text-xs font-mono" style={{ color: '#64748B' }}>
          {t('interests_section.f1.team') || 'Team favorito:'}
        </span>
        <span
          className="text-xs font-mono font-bold px-2.5 py-1 rounded-full"
          style={{ background: '#9CA3AF22', color: '#9CA3AF', border: '1px solid #9CA3AF40' }}
        >
          Cadillac 🛡️
        </span>
        <span className="text-xs font-mono" style={{ color: '#64748B' }}>
          {t('interests_section.f1.driver') || 'Piloto favorito:'}
        </span>
        <span
          className="text-xs font-mono font-bold px-2.5 py-1 rounded-full"
      ...
      >
        ...
        >
          <span
            ...
            style={{ color: '#64748B' }}
          >
            {t('interests_section.f1.title') || 'Clasificación Pilotos 2026'}
          </span>
          <div className="flex items-center gap-2">
            <div
              className="w-1.5 h-1.5 rounded-full animate-pulse"
              style={{ background: loading ? '#FFD700' : '#00FF80' }}
            />
            <span className="text-[10px] font-mono" style={{ color: '#64748B' }}>
              {loading
                ? t('interests_section.f1.syncing') || 'syncing...'
                : t('interests_section.f1.live') || 'live via jolpica-f1'}
            </span>
          </div>
        </div>
        <div className="relative">
          {loading && (
            <div
              ...
              style={{ background: 'rgba(12,12,26,0.4)' }}
            >
              <span className="text-xs font-mono text-[#00E5FF] animate-pulse">
                {t('interests_section.f1.loading') || 'Cargando telemetría...'}
              </span>
            </div>
          )}
          {standings.map((row: any) => (
            ...
            >
              <span
                className="w-5 text-xs font-mono text-center"
                style={{ color: row.pos === 1 ? '#FFD700' : '#64748B' }}
              >
                {row.pos}
              </span>
              <div
                className="w-1 h-7 rounded-full flex-shrink-0"
                style={{ background: row.color || TEAM_COLORS[row.team] || '#888' }}
              />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold flex items-center gap-2 flex-wrap">
                  ...
                  {row.driver === 'Sergio Pérez' && (
                    ...
                    >
                      {t('interests_section.f1.fav') || '⭐ fav'}
                    </span>
                  )}
                </div>
              ...
              <div className="text-xs font-mono hidden sm:block" style={{ color: '#64748B' }}>
                {row.fastest}
              </div>
              <div
                className="text-sm font-bold font-mono w-10 text-right"
                style={{ color: row.pos === 1 ? '#FFD700' : '#E2E8F0' }}
              >
                {row.points}
              </div>
            </div>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {GAMES.map((game) => (
        <div
          key={game.title}
          className="rounded-xl p-4 transition-all duration-200 hover:-translate-y-0.5 relative overflow-hidden"
          style={{ border: '1px solid rgba(124,58,237,0.2)', background: 'rgba(124,58,237,0.04)' }}
        >
          {game.img && (
            <div
              className="absolute top-0 left-0 w-full h-16 opacity-30 z-0 mask-image-gradient"
              style={{
          ...
          <div className="relative z-10">
            <div className="flex items-start justify-between mb-3">
              ...
              <span
                className="text-xs font-mono px-2 py-0.5 rounded-full whitespace-nowrap ml-2"
                style={{ background: 'rgba(124,58,237,0.2)', color: '#A78BFA' }}
              >
                {game.platform}
              </span>
            </div>
            <h4 className="font-bold text-sm mb-1" style={{ fontFamily: "'Exo 2', sans-serif" }}>
              {game.title}
            </h4>
  ...
  return (
    <div className="space-y-2.5">
      {MOVIES.map((movie, i) => (
        <div
          key={movie.title}
          className="flex items-center gap-4 rounded-xl px-4 py-3 hover:-translate-x-0.5 transition-all duration-200"
          style={{ border: '1px solid rgba(0,229,255,0.1)', background: 'rgba(0,229,255,0.02)' }}
        >
          <span
            className="text-base font-black w-6 text-center font-mono"
            style={{ color: i < 3 ? '#FFD700' : '#64748B' }}
          >
            {i + 1}
          </span>
          <div className="flex-1 min-w-0">
  ...
  const pets = [
    {
      ...
      name: 'Roxy',
      emoji: '🐕',
      type: t('interests_section.pets_data.roxy.type') || 'Perrita',
      desc:
        t('interests_section.pets_data.roxy.desc') ||
        'La guardiana de la casa. Energía infinita, amor incondicional. Fan del balón y de los paseos interminables.',
      img: 'https://images.unsplash.com/photo-1537151608828-ea2b11777ee8?w=500&h=500&fit=crop&auto=format',
      color: '#FF6B35',
      fact:
        t('interests_section.pets_data.roxy.fact') ||
        'Duerme en tu cama y ni siquiera te pide permiso',
    },
    {
      key: 'cat1',
      name: 'Nova',
      emoji: '🐱',
      type: t('interests_section.pets_data.nova.type') || 'Gata',
      desc:
        t('interests_section.pets_data.nova.desc') ||
        'La reina del hogar. Experta en interrumpir videollamadas y sentarse justo encima del teclado.',
      img: 'https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?w=500&h=500&fit=crop&auto=format',
      color: '#FFD700',
      fact:
        t('interests_section.pets_data.nova.fact') ||
        'Sabe exactamente cuándo abres una bolsa de snacks',
    },
    {
      key: 'cat2',
      name: 'Sooky',
      emoji: '🐱',
      type: t('interests_section.pets_data.sooky.type') || 'Gata',
      desc:
        t('interests_section.pets_data.sooky.desc') ||
        'La traviesa oficial. Especialista en tirar cosas de las mesas y pedir comida a las 3am.',
      img: 'https://images.unsplash.com/photo-1495360010541-f48722b34f7d?w=500&h=500&fit=crop&auto=format',
      color: '#C084FC',
      fact:
        t('interests_section.pets_data.sooky.fact') ||
        'Ha destruido exactamente 3 pares de auriculares',
    },
  ];
          ...
          <div className="p-4 space-y-2.5">
            <div className="flex items-center justify-between">
              <h3
                ...
                style={{ fontFamily: "'Exo 2', sans-serif", color: pet.color }}
              >
                {pet.emoji} {pet.name}
              </h3>
              <span className="text-xs font-mono" style={{ color: '#64748B' }}>
            ...
            <div
              ...
              style={{ background: pet.color + '12', color: pet.color + 'CC' }}
            >
              💡 {pet.fact}
            </div>
          </div>
    ...
    <section id="interests" className="py-28 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <h2 className="mt-4 text-3xl font-black" style={{ fontFamily: "'Exo 2', sans-serif" }}>
          {t('interests_section.title') || 'Fuera del código'}
        </h2>
        ...
        <div className="mt-8 flex gap-2 flex-wrap">
          {(
            [
              {
                key: 'f1',
                label: `🏎️  ${t('interests_section.tabs.f1') || 'Fórmula 1'}`,
                color: '#FF003C',
              },
              {
                key: 'games',
                label: `🎮  ${t('interests_section.tabs.gaming') || 'Juegos'}`,
                color: '#7C3AED',
              },
              {
                key: 'movies',
                label: `🎬  ${t('interests_section.tabs.cinema') || 'Películas'}`,
                color: '#00E5FF',
              },
              {
                key: 'pets',
                label: `🐾  ${t('interests_section.tabs.pets') || 'Mascotas'}`,
                color: '#FF6B35',
              },
            ] as const
          ).map((tab) => {
